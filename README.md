# NeetCode Roadmap 题解仓库 🧠

跟着 [NeetCode.io Roadmap](https://neetcode.io/roadmap) 刷题，记录每道题的思路与解法。

## 📚 分类导航

- 📦 [数组与哈希（Arrays & Hashing）](./01_array&hashing.md) ✅ 已完成
- 👣 [双指针（Two Pointers）](./02_two_pointers.md) ⬅️ 进行中
- 🪟 [滑动窗口（Sliding Window）](./03_sliding_window.md)
- 📐 [栈（Stack）](./04_stack.md)
- 🔍 [二分查找（Binary Search）](./05_binary_search.md)
- 🔗 [链表（Linked List）](./06_linked_list.md)
- 🌳 [二叉树（Trees）](./07_trees.md)
- 🌲 [前缀树（Tries）](./08_tries.md)
- ⛰️ [堆与优先队列（Heap / Priority Queue）](./09_heap.md)
- 🎯 [回溯（Backtracking）](./10_backtracking.md)
- 🕸️ [图（Graphs）](./11_graphs.md)
- 🧭 [高级图论（Advanced Graphs）](./12_advanced_graphs.md)
- ⚡ [一维动态规划（1-D DP）](./13_1d_dp.md)
- ⚡ [二维动态规划（2-D DP）](./14_2d_dp.md)
- 🦊 [贪心（Greedy）](./15_greedy.md)
- 📊 [区间（Intervals）](./16_intervals.md)
- 🔢 [数学与几何（Math & Geometry）](./17_math_geometry.md)
- 🧮 [位运算（Bit Manipulation）](./18_bit_manipulation.md)

---

## 🚀 进度

| 分类 | 状态 |
| --- | --- |
| Arrays & Hashing | 🟢 已完成 |
| Two Pointers | 🟡 进行中 |

---
